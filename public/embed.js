/**
 * MeetMe Embed Script v1.0
 * Provides floating button and popup functionality for embedding MeetMe scheduling.
 */
(function () {
  "use strict";
  ...
  var MeetMe = {
    ...
    showPopup: function (url) {
      var existing = document.getElementById("meetme-popup-overlay");
      if (existing) {
        existing.remove();
      }

      var overlay = document.createElement("div");
      overlay.id = "meetme-popup-overlay";
      overlay.style.cssText = [
        "position:fixed", "top:0", "left:0", "width:100%", "height:100%",
        "background:rgba(0,0,0,0.55)", "z-index:999999",
        "display:flex", "align-items:center", "justify-content:center",
        "backdrop-filter:blur(4px)", "-webkit-backdrop-filter:blur(4px)",
        "animation:meetme-fadein 0.2s ease"
      ].join(";");
      ...
      modal.style.cssText = [
        ...
        "animation:meetme-slidein 0.25s ease"
      ].join(";");
      ...
      if (!document.getElementById("meetme-styles")) {
        var style = document.createElement("style");
        style.id = "meetme-styles";
        style.textContent = [
          "@keyframes meetme-fadein{from{opacity:0}to{opacity:1}}",
          "@keyframes meetme-slidein{from{opacity:0;transform:translateY(16px) scale(0.97)}to{opacity:1;transform:none}}"
        ].join("");
    ...
    initFloatingButton: function (options) {
      options = options || {};
      var url = options.url;
      if (!url) { console.warn("[MeetMe] initFloatingButton: 'url' is required."); return; }
      ...
      var existing = document.getElementById("meetme-floating-btn");
      if (existing) existing.remove();

      var btn = document.createElement("button");
      btn.id = "meetme-floating-btn";
      ...
      btn.onclick = function () { MeetMe.showPopup(url); };
