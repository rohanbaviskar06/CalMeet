  ...
  return (
    <>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        ...
        <Button onClick={() => setIsDialogOpen(true)} className="gap-2 shadow-sm rounded-full px-6 w-full sm:w-auto justify-center">
      ...
      <div className="space-y-3">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-1">
                  ...
                  {workflow.isPremium && (
                    <Badge variant="secondary" className="bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400 text-[10px] font-bold tracking-widest px-2 py-0">PRO</Badge>
                  )}
                </div>
        ...
        >
          ...
          <span className="font-semibold text-sm uppercase tracking-widest">Create Custom Workflow</span>
        </button>
      </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="sm:max-w-[500px] rounded-[2rem]">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold">{editingWorkflow ? "Edit Workflow" : "Create Workflow"}</DialogTitle>
            <DialogDescription className="font-semibold uppercase text-[10px] tracking-widest">
              {editingWorkflow ? "Modify your automation" : "Set up a new automated action"}
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-6 mt-4">
            <div className="space-y-2">
              <label className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground ml-1">Workflow Name</label>
              <input 
                name="name"
                type="text" 
                defaultValue={editingWorkflow?.name || ""}
                placeholder="e.g. 48hr Reminder" 
                className="w-full px-4 py-3 rounded-2xl border bg-background font-medium focus:ring-2 focus:ring-primary/20 outline-none transition-all"
                required
              />
            </div>
            ...
            <div className="space-y-4 border p-5 rounded-[1.5rem] bg-muted/30">
              <div className="space-y-2">
                <label className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                  <Clock className="h-4 w-4" /> WHEN (Trigger)
                </label>
                <select 
                  name="trigger" 
                  className="w-full px-4 py-3 rounded-xl border bg-background font-medium outline-none" 
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-semibold uppercase tracking-widest text-primary flex items-center gap-2">
        ...
        <DialogContent className="sm:max-w-[550px] rounded-[2rem] p-6">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold flex items-center gap-2">
              <Zap className="h-5 w-5 text-primary" /> How Workflows Work
            </DialogTitle>
            <DialogDescription className="font-medium text-xs uppercase tracking-widest">
              Learn how to automate your meeting communications
            </DialogDescription>
          </DialogHeader>
          <div className="my-4 space-y-4">
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">
              Workflows allow you to automate standard communications surrounding your events. You define a **Trigger** (when to act) and an **Action** (what to do).
            </p>
            <div className="space-y-3">
              <div className="flex gap-3">
                ...
                <div>
                  <h4 className="text-sm font-semibold">Choose a Trigger</h4>
                  <p className="text-xs text-muted-foreground">Select when your workflow runs, such as right when an event is booked, canceled, or a set time before/after the meeting starts.</p>
                </div>
              </div>
              <div className="flex gap-3">
                ...
                <div>
                  <h4 className="text-sm font-semibold">Choose an Action</h4>
                  <p className="text-xs text-muted-foreground">Configure the automated communication, such as sending emails or SMS to your guests or yourself, or calling webhooks for integrations.</p>
                </div>
              </div>
              <div className="flex gap-3">
                ...
                <div>
                  <h4 className="text-sm font-semibold">Enable & Relax</h4>
                  <p className="text-xs text-muted-foreground">Switch the workflow to Active. The system automatically executes it whenever the conditions are met.</p>
                </div>
              </div>
            </div>
          </div>
